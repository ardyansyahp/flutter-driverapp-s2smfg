...
class TripIncidentScreen extends StatefulWidget {
  final ShippingTripModel trip;
  const TripIncidentScreen({super.key, required this.trip});

  @override
  State<TripIncidentScreen> createState() => _TripIncidentScreenState();
}

class _TripIncidentScreenState extends State<TripIncidentScreen> {
  ...
  Future<void> _submit() async {
    ...
    final result = await ctrl.reportIncident(
      ...
      customKeterangan: _selectedKendala == 'Lainnya' ? _customController.text.trim() : null,
      foto: _foto,
      lat: null,
      lng: null,
    );
                  ...
                  children: [
                    const Row(
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('Info Perjalanan',
                            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                      ],
                    ),
                    const Divider(height: 20),
                    _infoRow('Truck', widget.trip.actualTruck?.nopol ?? '-'),
                    _infoRow('Driver', widget.trip.actualDriver?.nama ?? '-'),
                    _infoRow('Trip ID', '#${widget.trip.id}'),
                  ],
                ),
              ),
            ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    RadioGroup<String>(
                      ...
                      onChanged: (val) => setState(() => _selectedKendala = val),
                      child: Column(
